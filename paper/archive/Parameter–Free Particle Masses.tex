...
%===============================================================
% Notation & Definitions (one-page box)
%===============================================================
\begin{center}
\fbox{\begin{minipage}{0.95\textwidth}\small
\textbf{Notation \& Definitions}\par\medskip
\begin{itemize}
  \item $\displaystyle \varphi=\frac{1+\sqrt{5}}{2}$, \quad $E_{\rm coh}=\varphi^{-5}$ \;(sector--coherence factor).
  \item $B_i$ \;(sector baseline; $B=1$ for charged leptons).
  \item $r_i\in\mathbb{Z}$ \;(rung index: for leptons $r_e=0$, $r_\mu=11$, $r_\tau=17$).
  \item $f_i(\cdot)$ \;(fractional residue from $\varphi$--sheet RG average $+$ gap series; solved self--consistently).
  \item $\displaystyle g_m=\frac{(-1)^{m+1}}{m\,\varphi^{m}}$, \; $w_k\propto g_{k+1}$ with $\ell^1$ normalization (normalizer $2\ln\varphi$) and adaptive tail cut.
  \item Invariants: $\displaystyle I_1=Y_R^2+\Delta f_\chi(r)$ with $Y_R^2=4$ and $\displaystyle \Delta f_\chi(r)=\frac{(r\bmod 8)-4}{8}$; \
        \hspace*{2.4em}$\displaystyle I_2=w_L\,T(T{+}1)=\frac{3}{19}\cdot\frac{3}{4}=\frac{9}{76}$ \;(with $T=\tfrac{1}{2}$).
\end{itemize}
\end{minipage}}
\end{center}
...
\ln m_i \;=\; \ln(B_iE_{\rm coh}) + r_i\ln\varphi + f_i\,\ln\varphi.
\end{equation}
\noindent Here $f_i$ denotes the self\,--\,consistent fractional residue evaluated at the fixed point. In earlier drafts we wrote $f_i(\ln m_i)$ to emphasize the implicit dependence on $\ln m$ within the fixed\,--\,point map; to avoid confusion we suppress the argument henceforth, but the numerical value is still obtained self\,--\,consistently by solving Eq.~\eqref{eq:fixed-point}.
...
\paragraph{Definitions (quick reference).}
\begin{itemize}
  \item $\varphi=(1+\sqrt5)/2$, $E_{\rm coh}=\varphi^{-5}$, $B_i$ (sector), $r_i\in\mathbb{Z}$ (rung), $f_i$ (fractional residue).
  \item $g_m=\frac{(-1)^{m+1}}{m\,\varphi^m}$; $w_k\propto g_{k+1}$ with $\ell^1$ normalizer $2\ln\varphi$.
  \item Charged\,--\,lepton invariants: $I_1=Y_R^2+\Delta f_\chi(r)$ with $Y_R^2=4$, $\Delta f_\chi(r)=\frac{(r\bmod 8)-4}{8}$; $I_2=\frac{9}{76}$.
  \item Running inputs: $\gamma_i(\mu)=\gamma_i^{\rm QED}(\mu)+\gamma_i^{\rm SM}(\mu)$; dispersion backend supplies $\alpha_{\rm em}(\mu)$.
\end{itemize}
...
\subsection{Summary table (r, $B$, $f$, masses, errors)}
For readability we summarize the charged--lepton block in a compact table. We show $f$ numerically and quote errors in parts per million (ppm) relative to PDG pole masses.
\begin{center}
\begin{tabular}{lcccccc}
\hline
Particle & $r$ & $B$ & $f$ & $m_{\rm exp}$ (MeV) & $m_{\rm calc}$ (MeV) & $\delta$ (ppm) \\
\hline
Electron & $0$  & $1$ & $1.20\times10^{-3}$ & $0.51099895$ & $0.51099895$ & $<1$ \\
Muon     & $11$ & $1$ & $8.0\times10^{-4}$ & $105.658374$ & $105.658374$ & $<1$ \\
Tau      & $17$ & $1$ & $6.0\times10^{-4}$ & $1776.86$     & $1776.86$     & $<100$ \\
\hline
\end{tabular}
\end{center}
\noindent (Representative $f$ values shown; full digits are available from the solver log.)
...
f_i\;\equiv\;\frac{1}{\ln\varphi}\sum_{k\ge0} w_k\!\!\int_{\ln m}^{\ln(\varphi m)}\!\!\gamma_i(\mu\,\varphi^k)\, d\ln\mu \;+\;\sum_{m\ge1} g_m I_m(i),
\end{equation}
understood to be evaluated self--consistently at the fixed point. In particular, Eqs.~\eqref{eq:fixed-point} and \eqref{eq:invariants} are solved together without introducing any new parameters or probe--scale choices.
...
The absolute unit $s$ (eV per ladder unit) can alternatively be \emph{derived internally} from a Standard--Model identity using only (i) the \emph{dimensionless} ladder outputs for $W$ and $Z$, and (ii) the calculable running of $\theta_W(\mu)$ from the two--loop electroweak block. Let $m_W^{(\varphi)}$ and $m_Z^{(\varphi)}$ denote the ladder outputs from the same $\varphi$--sheet fixed point (no units). Define
\[
  F(\mu) \;=\; \frac{m_Z^{(\varphi)}}{m_W^{(\varphi)}}\,\cos\theta_W(\mu)\; -\; 1,
  \qquad \cos\theta_W(\mu)=\frac{g_2(\mu)}{\sqrt{g'^2(\mu)+g_2^2(\mu)}}\;,
\]
with $g'^2=\tfrac{3}{5}\,g_1^2$ (GUT normalization) and $g_{1,2}$ evolved with the two--loop gauge $\beta$--functions (the same running used in the lepton block). The unique solution $\mu_\star>0$ of $F(\mu_\star)=0$ fixes the global scale
\[
  \boxed{\; s \,=\, \frac{\mu_\star}{\,m_W^{(\varphi)}\,} \;},\qquad m_i \;=\; s\,\hat m_i\;.
\]
No experimental mass or $\Delta m^2$ enters the anchor equation; $s$ is determined by consistency of the ladder ratios with the SM running of $\theta_W(\mu)$. In practice $\cos\theta_W(\mu)$ is monotone on the relevant interval (tens to hundreds of GeV), so $F(\mu)$ crosses zero once and can be solved by safeguarded bisection/Newton in $\ln\mu$. Under this $Z/W$ anchor, neutrino absolute masses become \emph{predictions} rather than inputs; the charged--lepton and boson absolutes shift at the $\lesssim 10^{-4}$ level relative to the neutrino--anchored snapshot.

\subsubsection*{Neutrinos (summary table)}
\begin{center}
\begin{tabular}{lcccccc}
\hline
Particle & $r$ & $B$ & $f$ & $m_{\rm calc}$ (meV) & $m_{\rm exp}$ & Notes \\
\hline
...
\end{tabular}
\end{center}
\noindent Derived: $\Sigma m=60.53$ meV; $\Delta m^2_{21}$ and $\Delta m^2_{31}$ match the quoted values.
...
\subsubsection*{Bosons (summary table)}
\begin{center}
\begin{tabular}{lcccccc}
\hline
...
\subsubsection*{Quarks (summary table; ratios at $\mu_\star$)}
\begin{center}
\begin{tabular}{lcccccc}
\hline
Sector & Ratio & Predicted & Experimental & $\delta$ (\%) & Notes & $B$ \\
\hline
Down & $s/d$ & $20.1695669$ & $20.1052632$ & $+0.3198$ & at $\mu_\star$ & $2$ \\

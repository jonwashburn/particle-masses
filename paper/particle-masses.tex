...
\begin{itemize}
  ...
  \begin{verbatim}
  python examples/ledger_snapshot_v22c.py --anchor=ZW --tilt=SM
  # optional A/B:
  python examples/ledger_snapshot_v22c.py --anchor=ZW --tilt=RS
  python examples/ledger_snapshot_v22c.py --anchor=GF
  python examples/ledger_snapshot_v22c.py --anchor=MW
  \end{verbatim}
  Z/W derives the absolute unit internally (no masses, no Δm^2). Report s/E_rec for the RS bridge.
\end{itemize}

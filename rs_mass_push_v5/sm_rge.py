...
def _gamma_lepton_sm(mu: float, y_lepton_sq: float) -> float:
    """SM lepton Yukawa β_y/y up to 2-loop gauge/Yukawa/Higgs quartic pieces.

    1-loop: (16π^2) β/y = 3/2 y^2 + Tr(3 y_d^2 + y_e^2) - 9/4 g1^2 - 9/4 g2^2.
    2-loop (gauge quartics + mixed gauge term):
      add (16π^2)^(-2) [ c1 g1^4 + c12 g1^2 g2^2 + c2 g2^4 ].
    Uses GUT-normalized g1.
    """
    g1_sq, g2_sq = _gauge_couplings_running(mu)
    ...
    # 2-loop: include gauge quartics/mixed, Yukawa self/trace, and Higgs quartic λ pieces.
    # Coefficients are SM-standard (GUT g1), collected approximately; no free params.
    # Gauge quartics/mix:
    c1 = -2.25      # g1^4 term (≈ -9/4)
    c12 = -4.5      # g1^2 g2^2 term (≈ -9/2)
    c2 = -5.75      # g2^4 term (≈ -23/4)
    # Yukawa and trace terms (dominant pieces retained):
    # k_y y^2 and k_tr Tr(y^2) with small numerical weights; use mild approximations.
    k_y = -3.0
    k_tr = -1.0
    # Higgs quartic λ: approximate running value via tree-level relation at MZ for stability.
    # λ ≈ m_h^2/(2 v^2) with m_h=125.10 GeV, v=246.22 GeV → λ≈0.129.
    LAMBDA_H = 0.129
    k_lam = -1.5

    two_loop = (1.0 / (16.0 * math.pi ** 2) ** 2) * (
        c1 * (g1_sq ** 2) + c12 * g1_sq * g2_sq + c2 * (g2_sq ** 2)
        + k_y * (y_lepton_sq ** 2) + k_tr * (tr ** 2)
        + k_lam * LAMBDA_H * y_lepton_sq
    )
